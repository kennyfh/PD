...
-- elemNivel :: Arbol a -> Int -> Maybe a
elemNivel a x
    | null nl = Nothing
    | otherwise = minimum nl
    where ls = getprofundidad x 0 a
          nl = deleteNothing ls


deleteNothing xs =  filter (/=Nothing) xs

getprofundidad :: Int -> Int -> Arbol Int -> [Maybe Int]
getprofundidad x prof (H n) = if (x==n) then [Just prof] else [Nothing]
getprofundidad x prof (N izq der) = getprofundidad x (prof+1) izq ++ getprofundidad x (prof+1) der
...
comprueba pila
    | esVacia pila = True
    | esVacia $ desapila pila = True
    | otherwise = (cima pila) < (cima $ desapila pila) && comprueba (desapila pila)
