...
data Arbol a =  H a | N (Arbol a) (Arbol a) deriving (Show,Eq)
...
profundidad (H a) = 0
profundidad (N izq der) = 1 + (profundidad izq) + (profundidad der)

elemento :: (Eq a) => a -> Int -> Arbol a -> Bool
elemento x n (H a) = x == a
elemento x 0 _ = False
elemento x n (N izq der) = (elemento x (n-1) izq) || (elemento x (n-1) der)

-- elemNivel :: Arbol -> Int -> Maybe Int
elemNivel ar x = checkElement [elemento x p ar| p<-[0..profundidad ar]]

-- anchura a = maximum [anchuraNivel p a | p <-[0..profundidad]

checkElement :: [Bool] -> Maybe Int
checkElement xs = if ((length ls) > 0) then head ls else Nothing
    where z = zip [0..] xs
          ls = [Just a | (a,b) <- z, b==True]
