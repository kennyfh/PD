...
data ArbolTO a = H | N a (ArbolTO a) (ArbolTO a) (ArbolTO a) deriving (Show,Eq)
...
arbolTOrdenado (N a i m d) = (funcion (<a) i) && (funcion (==a) m) && (funcion (>a) d) && arbolTOrdenado i && arbolTOrdenado d

funcion p (H) = True
funcion p (N a i _ d) = p a && (funcion p i) && (funcion p d)
