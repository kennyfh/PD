...
profundidad (H) = 0
profundidad (N izq n der) = 1 + max (profundidad izq) (profundidad der)
...
aux xs = [(a,b) | (a,b) <- ls, isJust a]
        where ls = zip xs [0..]

siguienteTurno xs = snd $ minimum (aux xs)
...
-- [Cerrada,Abierta (C [1,3,4]),Cerrada,Abierta (C [2,5])]
