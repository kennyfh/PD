...
-- ---------------------------------------------------------------------
-- 1) 
data Ficha = Roja | Azul
  deriving Show
type Columna =  [Ficha]
data CuatroEnRaya = Raya [Columna]
    deriving Show

tablero :: CuatroEnRaya
tablero = Raya [[Roja,Azul],[],[Azul],[]]

-- 2)
colocaFicha :: Ficha -> Int -> CuatroEnRaya -> CuatroEnRaya
colocaFicha ficha fila t = Raya $ map (\(x,y) -> if(x == fila) then [ficha] ++ y else y) las
  where las =  zip [1..] (lista t)


lista :: CuatroEnRaya -> [Columna]
lista (Raya []) = []
lista (Raya (x:xs)) = [x] ++  (lista (Raya xs))


-- Prelude> zip [1..] [["Roja","Azul"],[],["Azul"],[]]
-- [(1,["Roja","Azul"]),(2,[]),(3,["Azul"]),(4,[])]
...
--    * Otro para hojas, que contengan un elemento de tipo par de a y c
...
planetaporDefecto = Planeta {name="none", diameter="none", population="", residents=[]}
