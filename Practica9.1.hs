...
filtraPila :: (a -> Bool) -> Pila a -> Pila a
filtraPila p pila
    | esVacia (desapila pila) = pila
    | p (cima pila) = apila (cima pila) (filtraPila p (desapila pila))
...
mapPila f pila
    | esVacia pila = pila
    | otherwise = apila (f (cima pila)) (mapPila f (desapila pila))
-- ---------------------------------------------------------------------
...
pertenecePila y pila
    | esVacia pila = False
    | (cima pila) == y = True
    | otherwise = pertenecePila y (desapila pila)
...
ordenadaPila :: (Ord a) => Pila a -> Bool
ordenadaPila p1
    | esVacia (desapila p1) = True
    | otherwise = (cima p1) < cima (desapila p1) &&  (ordenadaPila (desapila p1))
...
pila2Lista p1
    | esVacia p1 = []
    | otherwise = [(cima p1)] ++ (pila2Lista $ desapila p1)
...
ordenaInserPila :: (Ord a) => Pila a -> Pila a
ordenaInserPila p1 = lista2Pila $ sort $ pila2Lista p1
-- ---------------------------------------------------------------------
...
prop_ordenaInserPila p = (ordenaInserPila p) ==  (lista2Pila $ sort $ pila2Lista p)
...
nubPila :: (Eq a) => Pila a -> Pila a
nubPila p1 = lista2Pila $ nub $ pila2Lista p1
...
maxPila :: (Ord a) => Pila a -> a
maxPila p1 = maximum $ pila2Lista p1
